...
for cheaper pricing:
```
aws emr create-cluster --name 'Development Cluster' --applications Name=Hadoop Name=Spark Name=Livy Name=Zeppelin --region us-west-2 --ec2-attributes KeyName=AWS_ECS_Demo_2 --release-label emr-5.33.0 --service-role EMR_DefaultRole --ec2-attributes InstanceProfile=EMR_EC2_DefaultRole --instance-fleets InstanceFleetType=MASTER,TargetSpotCapacity=1,InstanceTypeConfigs=['{InstanceType=m5.xlarge,BidPrice=0.05}'] InstanceFleetType=CORE,TargetSpotCapacity=1,InstanceTypeConfigs=['{InstanceType=m5.xlarge,BidPrice=0.05}'] 
```
